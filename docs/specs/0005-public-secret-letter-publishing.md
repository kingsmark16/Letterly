...
`SecretLetterRenderModel` has the same fields and is consumed by `SecretLetterRenderer` at `apps/web/src/templates/secret-letter/renderer.tsx`. The template registry is the source of the field schema and the section allowlist. This slice allows no optional section content.
