...
The question editor will explain the journey as a connected flow: question, answer, and next step. The first persisted question is the base question and always appears first. New questions append after the existing flow, while an answer can continue in order, go to a named question, or finish the journey. The editor will use connected nodes, plain destination labels, a path summary, and explicit save feedback. Creators cannot reorder questions.
...
- **AC-1**: The owner question editor presents a connected flow. Each question node exposes its prompt, answer type, answer labels, and plain destination controls. Internal question keys and display order fields are not shown.
- **AC-2**: New questions receive an automatically generated key and append after the current flow. The server assigns the order inside the page transaction. The creator does not enter a key or numeric order.
...
- **AC-7**: The editor has no drag and drop, keyboard reorder, or reorder mutation controls. The first question is marked as the Base question and is rendered first. New questions append after it, and explicit branch targets remain connected to their answer nodes.
...
- **AC-10**: Changes to prompts, types, answer labels, destinations, or finish state retain the existing response impact confirmation and deletion behavior. Deleting a question normalizes the remaining order; if the base is deleted, the next earliest question becomes the new base.
...
- **AC-13**: Unit, API integration, and Playwright coverage proves key generation, automatic order, explicit finish, named branching, backward compatibility, invalid references, deletion protection, disabled reorder behavior, stale saves, response impact confirmation, desktop behavior, mobile behavior, keyboard use, and the visitor finish state.

## Options considered

### Option 1: Structured flow with explicit transitions

Keep the existing question records, then present each question as a connected node. Add a finish flag and map destinations to plain language choices. Append new nodes in server assigned order and do not expose reordering.
...
- A complex graph is still represented as a structured flow rather than a full canvas.
...
**Chosen option**: A connected flow builder with explicit transitions.

The editor will use the existing page question API and design system, add an `endsJourney` flag to questions and choices, and derive Continue in order from the existing published root order. The visitor renderer and submission validation will stop at an explicit finish transition. Question nodes are laid out in a responsive sequence with answer nodes beneath choice questions and connector lines to the resolved next step. This is a structured flow view, not a free form canvas: creators can edit content and branch destinations, but cannot move nodes manually.

The first question is the base because the API assigns a new question the next contiguous order while holding the page row lock. Updates ignore any client supplied display order. Deletes normalize remaining question orders in the same transaction, so the first remaining question is always the base.

**Implementation skills**: `vercel-react-best-practices` (`vercel-labs/agent-skills`, `.agents/skills/vercel-react-best-practices/`)

## Rationale

The existing list editor already has ownership checks, stale content versions, response impact protection, and tests. A connected flow redesign improves the language without discarding those safety boundaries. A free form canvas would spend most of the feature budget on interaction mechanics, while a wizard would make the whole journey harder to inspect.
...
| Create question   | Initial order                          | Highest existing order plus one, assigned by the API while the page row is locked |
...
Each answer node shows its current destination in plain language. The connected flow shows question number, prompt, answer label, and resolved destination. It is derived from the saved question list, uses the shared root ordering rule, never exposes keys or ids, and marks an invalid or unresolved path beside the affected node.
...
- The base question cannot be a branch target. This keeps the first created question as the only initial entry point.
...
- Display order is a contiguous zero based list maintained by create and delete operations. There is no creator reorder operation.
- Content changes keep existing response impact confirmation. Order normalization does not delete responses.
...
- Confirm that no reorder endpoint, controls, or mutation are available, verifies **AC-7** and **AC-10**.
- Delete the base question and confirm the earliest remaining question becomes the new base after reload, verifies **AC-7** and **AC-10**.
...
4. [x] Replace the question editor with connected flow nodes, plain destination selectors, base question labeling, path summary, empty examples, delete reference protection, and recoverable save feedback, satisfies **AC-1**, **AC-2**, **AC-6**, **AC-7**, **AC-8**, **AC-9**, and **AC-12**.
5. [x] Add API, unit, and desktop and mobile Playwright coverage for the full path, backward compatibility, invalid branches, response protection, ownership, keyboard use, reduced motion, fixed base order, and connected flow nodes, satisfies **AC-10**, **AC-11**, **AC-12**, and **AC-13**.
...
- Existing ownership, versioning, response privacy, and graph safety are reused.
...
- Existing explicit targets remain stable because the creator cannot reorder questions.
- No new environment variables or services are required.

## Follow-up

- [ ] Consider a free form graph canvas only after real creator feedback shows that the structured flow is insufficient.
